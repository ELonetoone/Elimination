...
public class GameScreen extends Group {
	...
	private void paintDiamonds() {
		...
		for (int i = 0; i < diamondCircles.length; i++) {
			...
			for (int j = 0; j < diamondCircles[i].length; j++) {
				...
				diamondCircles[i][j].setLayoutX(START_Y + DISTANCE * j);
				diamondCircles[i][j].setLayoutY(START_X + DISTANCE * i);
	...
	private void appearDiamonds() {
		...
		for (Point point : list) {
			...
			temp.setPoint(point);
			temp.setLayoutY(START_X + ((point.getX() - 2) * DISTANCE));
			temp.setLayoutX(START_Y + ((point.getY() - 2) * DISTANCE));
